...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # self.dwc = nn.Conv2d(in_channels=hidden_features, out_channels=hidden_features, kernel_size=5, groups=hidden_features, padding=5 // 2)

    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        # num = int(x.shape[1] ** 0.5)
        # x = rearrange(x, "b (w h) c -> b c w h", w=num, h=num)
        # x = rearrange(self.dwc(x), "b c w h -> b (w h) c")
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        relative_coords = coords_flatten.unsqueeze(-1) - coords_flatten.unsqueeze(-2)
