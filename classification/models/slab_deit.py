...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=False, attn_drop=0.1, proj_drop=0.1,
                 focusing_factor=3):
...
@register_model
def deit_tiny_patch16_224_linear_repbn(pretrained=False, **kwargs):
    ln = partial(nn.LayerNorm, eps=1e-6)
    linearnorm = partial(LinearNorm, norm1=ln, norm2=RepBN, step=60000)
    model = VisionTransformer(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=linearnorm, **kwargs)
...
@register_model
def deit_small_patch16_224_linear_repbn(pretrained=False, **kwargs):
    ln = partial(nn.LayerNorm, eps=1e-6)
    linearnorm = partial(LinearNorm, norm1=ln, norm2=RepBN, step=60000)
    model = VisionTransformer(
